...
def my_parallel_wrapper_fn_full(env_fn, seed=None):
    def par_fn(**kwargs):
        env = env_fn(**kwargs)
        env = aec_to_parallel_wrapper_full(env, seed)
...
# from utils.conversion.py
class aec_to_parallel_wrapper_full(aec_to_parallel_wrapper):
    def __init__(self, aec_env, seed=None):
        aec_env.reset()
        self.observation_space_ = batch_space(
            aec_env.unwrapped.observation_space_full("pursuer_0"), aec_env.num_agents
        )
    ...
    def reset(self, seed=None, return_info=False, options=None):
        ...
        observations = {
            agent: self.aec_env.unwrapped.observe_full(agent)
...
my_parallel_env_full = my_parallel_wrapper_fn_full(pursuit_v4.env)
