...
def test_dqn(args=get_args()):
    task_parameter = {'shared_reward': False, 'n_evaders': 3, 'freeze_evaders': True, 'n_pursuers': 3}
    ...
    train_envs = DummyVectorEnv(
        [lambda: MultiDiscreteToDiscrete(my_parallel_env(**task_parameter)) for _ in range(args.training_num)]
    )
    # test_envs = gym.make(args.task)
    test_envs = DummyVectorEnv(
        [lambda: MultiDiscreteToDiscrete(my_parallel_env(**task_parameter)) for _ in range(args.test_num)]
    )
    # seed
    ...
    if __name__ == '__main__':
        ...
        env = DummyVectorEnv([lambda: MultiDiscreteToDiscrete(my_parallel_env(render_mode='human', **task_parameter))])
